...
void projectPart(string workDir, string distribDir, int partition, int section, int graphLevel, int maxIter)
{
	...
	for(int i = 0; i < hybSize1; i++)
	{
		...
		if(cPart.find(currNode) != cPart.end())
		{
			...
			if(childMap[2*i+1] != -1 && prevPart.find(hybMap2[childMap[2*i+1]]) != prevPart.end())
	...
	prevPart.clear();
	delete [] hybMap1; delete [] hybMap2; delete [] childMap;


	//FINISH HERE LOAD nDEN AND LOAD GRAPHS 
	sprintf(fileName, "%s/HybSpectrum/Graph%d/nDen", workDir.c_str(), graphLevel-1);
	pFile = fopen(fileName, "r");
	...
	sprintf(fileName, "%s/HybSpectrum/Graph%d", workDir.c_str(), graphLevel-1);
	...
	for(int i = 0; i < graphsLoaded; i++)
	{
		...
		char * fileName = new char [200];
		sprintf(fileName, "%s/HybSpectrum/Graph%d/graph%d", workDir.c_str(), graphLevel-1, i);
