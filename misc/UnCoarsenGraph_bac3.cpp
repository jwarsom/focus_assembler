...
void removeContainments(Graph ** &, int, uint32_t * &, uint32_t * &, int * &, int, int, int * &, int, int);
	...
	if(rank == 0)
	{
		...
		while(true)
		{
			...
			if(status.MPI_TAG == assembleTag)
			{
				...
				uint32_t * newEdges = new uint32_t [5000];
					...
					{	
						...
						if(assembleSet.find(nIndex) != assembleSet.end())
						{
							...
							if(numNodes > 1000)
							{
								...
								delete  newEdges; uint32_t * newEdges = new uint32_t [5*numNodes];
							}
							...
							removeContainments(graphs, graphsLoaded, cMap, sandBox, contigs, startPos, count, nodeMap, minNode, maxNode);
							...
							for(int j = startPos; j < count && contigs[j] != -1; j++)
							{
								...
								newEdges[5*cMap[currNode-minNode]] = 0; 
							...
							while(currContig < numContigs)
							{
								...
								cout<<contigLen<<" "<<totalContigs<<" "<<currContig<<endl;
							...
							if(numNodes > 1000)
							{
								...
								delete [] newEdges; newEdges = new uint32_t [5000];
							}
						}
...
//Output:None
//Return:None
void removeContainments(Graph ** & graphs, int graphsLoaded, uint32_t * & cMap, uint32_t * & sandBox, int * & contigs, int startPos, int count, int * & nodeMap, int minNode, int maxNode)
{
	int currLabel = 0; 
	for(int i = startPos; i < count && contigs[i] != -1; i++)
	{
