...
void removeContainments(Graph ** &, int, uint32_t * &, uint32_t * &, uint32_t * &, int * &, int, int, int * &, int, int);
void transitiveReduce(Graph ** &, int, uint32_t * &, uint32_t * &, uint32_t * &, int * &, int, int, int * &, int, int);
void transitiveReduce(uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, int);
void fillEdges(Fragment_Index ** &, int, uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, int ** &, int);
int trimGraph(uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, int, int);
int popBubbles(uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, int, int);
int traverseGraph(uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, uint32_t * &, int);
	...
	if(rank == 0)
	{
		...
		while(true)
		{
			...
			if(status.MPI_TAG == assembleTag)
			{
				...
				uint32_t * cMap = new uint32_t [1000];
				uint32_t * parents = new uint32_t[1000];
				...
				char * labelBuff = new char [10 * megabyte]; 
				int labelPos = 0;

				int tally = 0;

				char * tmpFile = new char [1000];
				sprintf(tmpFile, "%s/out.%d", workDir.c_str(), graphIter);
				...
				for(int i = 0; i < count; i++)
				{
					...
					{	
						...
						if(assembleSet.find(nIndex) != assembleSet.end())
						{
							...
							for(int j = startPos; j < count && contigs[j] != -1; j++)
								sandBox[100+(nodeMap[contigs[j]]-minNode)] = 0;
							...
							if(numNodes > 1000)
							{
								...
								delete [] cMap; uint32_t * cMap = new uint32_t [numNodes];
								delete [] parents; uint32_t * parents = new uint32_t [numNodes];
							...
							loadLabels(workDir, labels, lSize, labelsLoaded, currLabelSet, contigs, startPos, count, nodeMap);


							removeContainments(graphs, graphsLoaded, cMap, parents, sandBox, contigs, startPos, count, nodeMap, minNode, maxNode);
							...
							for(int j = startPos; j < count && contigs[j] != -1; j++)
							{
								...
								if(cMap[currNode-minNode] > parentNodes)
									parentNodes = cMap[currNode-minNode];

								nodes[cMap[currNode-minNode]] = 1;

								forwardEdges[5*cMap[currNode-minNode]] = 0;
								backEdges[5*cMap[currNode-minNode]] = 0;
								sandBox[100+cMap[currNode-minNode]] = 0;
							}

							parentNodes++;

							/*
							transitiveReduce(graphs, graphsLoaded, cMap, forwardEdges, backEdges, contigs, startPos, count, nodeMap, minNode, maxNode);

							trimGraph(nodes, forwardEdges, backEdges, sandBox, parentNodes,  minPathLen);
							popBubbles(nodes, forwardEdges, backEdges, sandBox, parentNodes, minPathLen);
							float numContigs = traverseGraph(nodes, forwardEdges, backEdges, orderings, sandBox, parentNodes);
							...
							if(maxContig/totalContigs > .80)
							{
								...
								for(int j = startPos; j < count && contigs[j] != -1; j++)
								{
									int currNode = nodeMap[contigs[j]];
									Labels * label;
									for(int k = 0; k < labelsLoaded; k++)
									{
										label = labels[k];
										if(label->getOffset() <= currNode && currNode < label->getOffset() + label->getNumLabels())
											break;
									}

									string l1 = label->labelAt(currNode-label->getOffset());
									if(l1.size() + labelPos + 4 >= 10 * megabyte)
									{

										FILE * tFile = fopen(tmpFile, "a");
										fwrite(labelBuff, sizeof(char), labelPos, tFile);									     fclose(tFile);
										labelPos = 0;
									}

									strcpy(labelBuff+labelPos, l1.c_str());
									labelPos+=(l1.size());
									labelBuff[labelPos++] = '\n';
								}
							...
							*/
							...
							if(numNodes > 1000)
							{
								...
								delete [] cMap; uint32_t * cMap = new uint32_t [1000];
								delete [] parents; uint32_t * parents = new uint32_t [1000];
...
//void fillEdges()
//Description: This function aligns reads within the graph to discover more edges
//Input: graph (graph &): the overlap graph, index (Fragment_Index * &) holds fragments
//Output:None
//Return:None
void transitiveReduce(uint32_t * & nodes, uint32_t * & forwardEdges, uint32_t * & backEdges, uint32_t * & sandBox, int numNodes)
{
		...
		if(nodes[i])
		{
			...
			if(fTally > 1)
			{
				...
				for(int j =0; j < forwardEdges[5*i]; j++)
				{
					if(nodes[forwardEdges[5*i+j+1]])
					{
						sandBox[listSize++] = forwardEdges[5*i+j+1];	
					}
				}

				int eraseSize = listSize;
				for(int j = 0; j < listSize; j++)
				{
					for(int k = 0; k < listSize; k++)
					{
						uint32_t node3 = sandBox[j];
						uint32_t node2 = sandBox[k];

						bool erased = false;
						for(int h = 0; h < forwardEdges[5*node2]; h++)
						{
							if(forwardEdges[5*node2+1+h] == node3)
							{
								sandBox[eraseSize++] = node3;
								erased = true; break;
							}	
						}
				...
				for(int j = listSize; j < eraseSize; j++)
				{
					uint32_t node1 = i; uint32_t node2 = sandBox[j];
					int swapPos = 0;
					for(int k = 0; k < forwardEdges[5*node1]; k++)
					{
						if(forwardEdges[5*node1+k+1] == node2)
						{
							swapPos = k; break;
						}
					}

					for(int k = swapPos+1; k < forwardEdges[5*node1]; k++)
					{
						forwardEdges[5*node1 + 1 + swapPos++] = forwardEdges[5*node1 + 1 + k];
					}

					forwardEdges[5*node1]--;

					swapPos = 0;
					for(int k = 0; k < backEdges[5*node2]; k++)
					{
						if(backEdges[5*node2+k+1] == node1)
						{
							swapPos = k; break;
						}
					}

					for(int k = swapPos+1; k < backEdges[5*node2]; k++)
					{
						backEdges[5*node2 + 1 + swapPos++] = backEdges[5*node2 + 1 + k];
					}
...
//Output:None
//Return:None
void fillEdges(Fragment_Index ** & indexes, int indexesLoaded, uint32_t * & nodes, uint32_t * & forwardEdges, uint32_t * & backEdges, uint32_t * & parents, uint32_t * & sandBox, int ** & matrix, int numNodes)
{
	for(int i = 0; i < numNodes; i++)
	{
		if(nodes[i])
		{
			...
			if(fTally > 1)
			{
				...
				for(int j = 0; j < listSize; j++)
				{
					for(int k = 0; k < listSize; k++)
					{
						if(sandBox[j] < sandBox[k])
						{
							int frag1 =  parents[sandBox[j]];
							int frag2 =  parents[sandBox[k]];

							Fragment_Index * qIndex; Fragment_Index * rIndex;
							for(int h = 0; h < indexesLoaded; h++)
							{
								qIndex = indexes[h];
								if(frag1 >= qIndex->getIndexOffset() && frag1 < qIndex->getIndexOffset() + qIndex->numFragments())
									break;
							}
							...
							{
								rIndex = indexes[h];
								if(frag2 >= rIndex->getIndexOffset() && frag2 < rIndex->getIndexOffset() + rIndex->numFragments())
									break;
							}

							pair<unsigned long long int, unsigned long long int> qBounds;
							qBounds = qIndex->indexBounds(frag1-qIndex->getIndexOffset());	
							pair<unsigned long long int, unsigned long long int> rBounds;
							rBounds = rIndex->indexBounds(frag2-rIndex->getIndexOffset());	

							int xLen = qBounds.second - qBounds.first;
							int yLen = rBounds.second - rBounds.first;

							int band = max(xLen, yLen);
							BandedAlignment<Fragment_Index> align(qBounds.first, rBounds.first, xLen, yLen, band, true);
							pair<float, float> alignScore;
							if(xLen <= 1000 && yLen <= 1000)
							{
								alignScore = align.align(matrix, *qIndex, *rIndex, 0);
							}else{
								...
								alignScore = align.align(largeMatrix, *qIndex, *rIndex, 0);
							...
							if(((alignScore.first - alignScore.second)/alignScore.first) >= .90 && alignScore.first >= 50)
							{
								float cInterval = .03;

								float qLen = qBounds.second-qBounds.first; 
								float rLen = rBounds.second-rBounds.first;

								uint32_t sContained = (alignScore.first/qLen + cInterval);
								uint32_t dContained = (alignScore.first/rLen + cInterval); 
								...
								if(sContained && dContained && qLen >= rLen)
									nodes[node2] = 0;

								if(sContained && dContained && rLen > qLen)
									nodes[node1] = 0;

								if(!sContained && !dContained)
								{
									if((align.getXStart() < align.getYStart()))
									{
										int numEdges1 = forwardEdges[5*node1];
										int numEdges2 = backEdges[5*node2];

										bool foundEdge = false;
										for(int h = 0; h < numEdges1; h++)
										{
											if(forwardEdges[5*node1+h+1] == node2)
											{
												foundEdge = true; break;
											}
										}	

										for(int h = 0; h < numEdges2; h++)
										{
											if(backEdges[5*node2+h+1] == node1)
											{
												foundEdge = true; break;
											}
										}	

										if(numEdges1 < 4 && numEdges2 < 4 && !foundEdge)
										{
											forwardEdges[5*node1+1+numEdges1] = node2;
											backEdges[5*node2+1+numEdges2] = node1;
											forwardEdges[5*node1]++;
											backEdges[5*node2]++;
										...
										int numEdges1 = forwardEdges[5*node2];
										int numEdges2 = backEdges[5*node1];

										bool foundEdge = false;
										for(int h = 0; h < numEdges1; h++)
										{
											if(forwardEdges[5*node2+h+1] == node1)
											{
												foundEdge = true; break;
											}
										}	
										...
										for(int h = 0; h < numEdges2; h++)
										{
											if(backEdges[5*node1+h+1] == node2)
											{
												foundEdge = true; break;
											}
										}	
										...
										if(numEdges1 < 4 && numEdges2 < 4 && !foundEdge)
										{
											forwardEdges[5*node2+1+numEdges1] = node1;
											backEdges[5*node1+1+numEdges2] = node2;
											forwardEdges[5*node2]++;
											backEdges[5*node1]++;
										}
									}
								}	
			...
			if(bTally > 1)
			{
				...
				for(int j =0; j < backEdges[5*i]; j++)
				{
					if(nodes[backEdges[5*i+j+1]])
					{
						sandBox[listSize++] = backEdges[5*i+j+1];	
					}
				}
...
//Output:None
//Return:None
int traverseGraph(uint32_t * & nodes, uint32_t * & forwardEdges, uint32_t * & backEdges, uint32_t * & orderings, uint32_t * & sandBox, int numNodes)
{	
	int numContigs = 0;
	...
	for(int i = 0; i < numNodes; i++)
	{
		if(nodes[i] && !sandBox[i+100])
		{
			...
			{		
				...
				fTally = 0; bTally = 0;
				for(int j = 0; j < forwardEdges[5*(nextNode)]; j++)
				{
					int n = forwardEdges[5*(nextNode)+j+1];
					if(nodes[n])
					{
						fIndex = j; fTally++;
					}
				...
				for(int j = 0; j < backEdges[5*(nextNode)]; j++)
				{
					int n = backEdges[5*(nextNode)+j+1];
					if(nodes[n])
					{
						bIndex = j; bTally++;
					}
				...
				while(!sandBox[(nextNode)+100] && fTally == 1 && bTally == 1)
				{
					int currPos = orderings[startPos];
					orderings[startPos+currPos+1] = nextNode;
					sandBox[(nextNode)+100] = 1;
					nextNode = forwardEdges[5*(nextNode)+1+fIndex];
				...
				if(!sandBox[(nextNode)+100] && bTally == 1)
				{
					int currPos = orderings[startPos];
					orderings[startPos+currPos+1] = nextNode;
					sandBox[(nextNode)+100] = 1;
			...
			if(bTally == 1)
			{
				...
				while(!sandBox[(nextNode)+100] && fTally == 1 && bTally == 1)
				{
					...
					orderings[startPos+currPos+1] = nextNode;
					sandBox[(nextNode)+100] = 1;
					nextNode = backEdges[5*(nextNode)+1+bIndex];
				...
				if(!sandBox[(nextNode)+100] && fTally == 1)
				{
					int currPos = orderings[startPos];
					orderings[startPos+currPos+1] = nextNode;
					sandBox[(nextNode)+100] = 1;
...
//Output:None
//Return:None
int trimGraph(uint32_t * & nodes, uint32_t * & forwardEdges, uint32_t * & backEdges, uint32_t * & sandBox, int numNodes, int pathLen)
{
		...
		if(nodes[i])
		{
			...
			if(fTally == 0 && bTally == 1)
			{
				...
				while(fTally == 1 && bTally == 1)
				{
					...
					nextNode = backEdges[5*(nextNode)+ bIndex + 1];
			...
			if(fTally == 1 && bTally == 0)
			{
				...
				{
					sandBox[numExplored++] = nextNode;
					nextNode = forwardEdges[5*(nextNode)+fIndex + 1];
...
//Output:None
//Return:None
int popBubbles(uint32_t * & nodes, uint32_t * & forwardEdges, uint32_t * & backEdges, uint32_t * & sandBox, int numNodes, int pathLen)
{
	int numPopped = 0;
	for(int i = 0; i < numNodes; i++)
	{
		if(nodes[i])
		{
			if(forwardEdges[i*5] == 2)
			{
				...
				if(nodes[node1] && nodes[node2])
				{
					...
					for(int j = 0; j < forwardEdges[5*(node1)]; j++)
					{
						int n = forwardEdges[5*(node1)+j+1];
						if(nodes[n])
						{
							fIndex1 = j; fTally1++;
						}
					...
					for(int j = 0; j < backEdges[5*(node1)]; j++)
					{
						int n = backEdges[5*(node1)+j+1];
						if(nodes[n])
						{
							bIndex1 = j; bTally1++;
						}
					...
					for(int j = 0; j < forwardEdges[5*(node2)]; j++)
					{
						int n = forwardEdges[5*(node2)+j+1];
						if(nodes[n])
						{
							fIndex2 = j; fTally2++;
						}
					...
					for(int j = 0; j < backEdges[5*(node2)]; j++)
					{
						int n = backEdges[5*(node2)+j+1];
						if(nodes[n])
						{
							bIndex2 = j; bTally2++;
						}
					...
					{
						node1 = forwardEdges[5*(node1)+1+fIndex1];
						node2 = forwardEdges[5*(node2)+1+fIndex2];
...
//Output:None
//Return:None
void transitiveReduce(Graph ** & graphs, int graphsLoaded, uint32_t * & cMap, uint32_t * & forwardEdges, uint32_t * & backEdges, int * & contigs, int startPos, int count, int * & nodeMap, int minNode, int maxNode)
{
	for(int i = startPos; i < count && contigs[i] != -1; i++)
	{
		...
		for(int j = graph1->nodeDegree(node1-graph1->getOffset())-1; j >= 0; j--)
		{
			...
			if(!graph1->rDovetail(node1-graph1->getOffset(), j) && node3 >= minNode && node3 <= maxNode && cMap[node1-minNode] != cMap[node3-minNode])
			{
				bool reduced = false;
				for(int k = 0; k < graph1->nodeDegree(node1-graph1->getOffset()); k++)
				{
					uint32_t node2 = graph1->edgeDest(node1-graph1->getOffset(), k);
					if(!graph1->rDovetail(node1-graph1->getOffset(), k) && node2 >= minNode && node2 <= maxNode && cMap[node1-minNode] != cMap[node2-minNode] && cMap[node2-minNode] != cMap[node3-minNode])
					{
						Graph * graph2;
						for(int l = 0; l < graphsLoaded; l++)
						{
						...
						{
							int checkNode = graph2->edgeDest(node2-graph2->getOffset(), h);
							if(!graph2->rDovetail(node2-graph2->getOffset(), h) && checkNode >= minNode && checkNode <= maxNode && cMap[checkNode- minNode] == cMap[node3-minNode])
							{
								reduced = true; break;
							}
						}
				...
				if(!reduced)
				{
					int numEdges1 = forwardEdges[5*cMap[(node1-minNode)]];
					int numEdges2 = backEdges[5*cMap[(node3-minNode)]];

					if(numEdges1 < 4 && numEdges2 < 4)
					{
						bool foundEdge1 = false; bool foundEdge2 = false;

						for(int k = 0; k < numEdges1; k++)
						{
							int p1 = k+5*cMap[(node1-minNode)]+1;
							if(forwardEdges[p1] == cMap[node3-minNode])
							{
								foundEdge1 = true; break;
							}
						}		

						for(int k = 0; k < numEdges2; k++)
						{
							int p1 = k+5*cMap[(node3-minNode)]+1;
							if(forwardEdges[p1] == cMap[node1-minNode])
							{
								foundEdge2 = true; break;
							}
						}		

						if(!foundEdge1 && !foundEdge2)
						{
							int pos1 = numEdges1+5*cMap[(node1-minNode)]+1;
							int pos2 = numEdges2+5*cMap[(node3-minNode)]+1;
							forwardEdges[pos1] = cMap[node3-minNode];
							backEdges[pos2] = cMap[node1-minNode];

							forwardEdges[5*cMap[node1-minNode]]++;
							backEdges[5*cMap[node3-minNode]]++;
						}
					}	
				}
			}				
...
//Output:None
//Return:None
void removeContainments(Graph ** & graphs, int graphsLoaded, uint32_t * & cMap, uint32_t * & parents, uint32_t * & sandBox, int * & contigs, int startPos, int count, int * & nodeMap, int minNode, int maxNode)
{
	int currLabel = 0; 
	for(int i = startPos; i < count && contigs[i] != -1; i++)
	{
		int currNode = nodeMap[contigs[i]];
		if(!sandBox[100+(currNode-minNode)])
		{
			int beginQueue = 0; int endQueue = 0;

			int parent = currNode;
			cMap[currNode-minNode] = currLabel;
			sandBox[100+(currNode-minNode)] = 1;
			sandBox[endQueue++] = parent;

			while(beginQueue != endQueue)
			{
				int currNode = sandBox[beginQueue++];
				Graph * graph;
				for(int j = 0; j < graphsLoaded; j++)
				{
				...
				for(int j = 0; j < graph->nodeDegree(currNode-graph->getOffset()); j++)
				{
					int newNode = graph->edgeDest(currNode-graph->getOffset(), j);	
					if(newNode >= minNode && newNode <= maxNode && !sandBox[100+(newNode-minNode)])
					{
						if(graph->dContained(currNode-graph->getOffset(), j))
						{
							sandBox[endQueue++] = newNode;
							sandBox[100+(newNode-minNode)] = 1;
							cMap[newNode-minNode] = currLabel;
						}

						if(currNode == parent && graph->sContained(currNode-graph->getOffset(), j))
						{
							if(!graph->dContained(currNode-graph->getOffset(), j))
							{
								sandBox[endQueue++] = newNode; 
			...
			parents[currLabel] = parent;
			currLabel++;
		}
	}

	for(int i = startPos; i < count && contigs[i] != -1; i++)
		sandBox[100+(nodeMap[contigs[i]]-minNode)] = 0;
}
